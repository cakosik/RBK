...
↘️ Выбери одну из <b>категорий</b>
    ...
    if user_status == 'Admin':
       ...
       await callback.message.edit_text(f"<a href='tg://user?id={user_id}'>{user_name}</a>,Вы не являетесь администратором бота 👮. Для получение прав администратора обратитесь к разработчику <a href='t.me/CheIovechek06/'>Chester</a>  ⚠️ ", parse_mode='html')
...
🤠 <b>ADMIN:</b> <code>{len(records2)}</code>
    ...
    if user_status == "Owner":
       ...
       await callback.message.answer(f"<a href='tg://user?id={user_id}'>{user_name}</a>,Вы не являетесь администратором бота 👮. Для получение прав администратора обратитесь к разработчику <a href='t.me/CheIovechek06/'>Chester</a>  ⚠️ ", parse_mode='html')
...
🛒 Для покупки администраторских прав , обратитесь к Владельцу бота - <a href='t.me/CheIovechek06/'>Chester</a> 
    """,  parse_mode='html' )
...
async def ava_admin_callback(callback: types.CallbackQuery):
   ...
   if user_status in ['Admin', 'Helper_Admin', 'Owner']:
      ...
      await callback.message.answer( f"<a href='tg://user?id={user_id}'>{user_name}</a>, вы не являетесь администратором бота 👮. Для получение прав администратора обратитесь к разработчику <a href='t.me/Listik_qwe/'>Listik</a>  ⚠️", parse_mode='html')
